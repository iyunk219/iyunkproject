    ...
    <tr>
        <td>
            <div style="display: inline-block; text-align: left; border: 1px solid #ccc; border-radius: 8px; padding: 20px; background-color: #f9f9f9;">
                ...
                <form action="{{url('/backend/admin/update/'.$admin->id)}}" method="post">
                    {{@csrf_field()}}
                    <div style="margin-bottom: 15px;">
                        <label for="product-name">Username</label><br>
                        <input type="text" id="username" name="username" value="{{$admin->username}}" required style="width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ccc;">
                    </div>
                    <div style="margin-bottom: 15px;">
                        <label for="product-price">Password</label><br>
                        <input type="text" id="password" name="password" value="{{$admin->password}}"required style="width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ccc;">
                    </div>
